...
MY_SUBSTRATE_X_LEN = 2.0
MY_SUBSTRATE_Y_LEN = 0.5
MY_SUBSTRATE_MATERIAL_DENSITY = 1e-9
MY_SUBSTRATE_MATERIAL_EMOD = 1e0
MY_SUBSTRATE_RAYLEIGH_DAMPING_COEFFICIENTS = (1e2, 0.0, 0.0, 0.0)  # (alpha, beta, composite, structural)
MY_SUBSTRATE_SHELL_THICKNESS = 0.1
MY_SUBSTRATE_MESH_SEED_SIZE = 0.025
...
MY_FOUTPUT_VARIABLES = ['U', 'S', 'LE', 'CSTRESS', 'CDISP', 'CSLIPR', 'CTANDIR', 'CFORCE', 'CTHICK', 'FSLIPR', 'FSLIP', 'CSTATUS']
# Note: Abaqus 6.14 does not support the outputs 'CSTATUS', 'CDISP', 'CTANDIR', 'CSLIPR'.

MY_STEP_1_FOUTPUT_NUM = 10
MY_STEP_2_FOUTPUT_NUM = 10
MY_STEP_3_FOUTPUT_NUM = 1000
MY_STEP_1_MASS_SCALING_FACTOR = 1e6
MY_STEP_2_MASS_SCALING_FACTOR = 1e8
MY_STEP_3_MASS_SCALING_FACTOR = 1e1
...
def M1030_create_structure_material_and_section():
    ...
    material.Damping(
        ...
        composite=MY_STRUCTURE_RAYLEIGH_DAMPING_COEFFICIENTS[2],
        structural=MY_STRUCTURE_RAYLEIGH_DAMPING_COEFFICIENTS[3],
    )
...
def M1070_create_substrate_material_and_section():
    ...
    material.Density(table=[[MY_SUBSTRATE_MATERIAL_DENSITY]])
    ...
    material.Damping(
        alpha=MY_SUBSTRATE_RAYLEIGH_DAMPING_COEFFICIENTS[0],
        beta=MY_SUBSTRATE_RAYLEIGH_DAMPING_COEFFICIENTS[1],
        composite=MY_SUBSTRATE_RAYLEIGH_DAMPING_COEFFICIENTS[2],
        structural=MY_SUBSTRATE_RAYLEIGH_DAMPING_COEFFICIENTS[3],
    )
...
def M1090_create_substrate_mesh():
    ...
    elem_type_1 = mesh.ElemType(elemCode=C3D8, elemLibrary=EXPLICIT)
...
def M1100_create_step():
    ...
    model.ExplicitDynamicsStep(name='Step-2', previous='Step-1')
    # Step-3: Release the prestrain, allowing the structure to assemble.
    model.ExplicitDynamicsStep(name='Step-3', previous='Step-2')

    model.steps['Step-1'].setValues(
        massScaling=[
            (SEMI_AUTOMATIC, MODEL, AT_BEGINNING, MY_STEP_1_MASS_SCALING_FACTOR, 0.0, None, 0, 0, 0.0, 0.0, 0, None),
        ]
    )
    model.steps['Step-2'].setValues(
        massScaling=[
            (REINITIALIZE, MODEL, AT_BEGINNING, 0.0, 0.0, None, 0, 0, 0.0, 0.0, 0, None),
            (SEMI_AUTOMATIC, MODEL, AT_BEGINNING, MY_STEP_2_MASS_SCALING_FACTOR, 0.0, None, 0, 0, 0.0, 0.0, 0, None),
        ]
    )
    model.steps['Step-3'].setValues(
        massScaling=[
            (REINITIALIZE, MODEL, AT_BEGINNING, 0.0, 0.0, None, 0, 0, 0.0, 0.0, 0, None),
            (SEMI_AUTOMATIC, MODEL, AT_BEGINNING, MY_STEP_3_MASS_SCALING_FACTOR, 0.0, None, 0, 0, 0.0, 0.0, 0, None),
        ]
    )
    ...
    field_output_request = model.FieldOutputRequest(
        ...
        numIntervals=MY_STEP_1_FOUTPUT_NUM,
    )
    field_output_request.setValuesInStep(stepName='Step-2', numIntervals=MY_STEP_2_FOUTPUT_NUM)
    field_output_request.setValuesInStep(stepName='Step-3', numIntervals=MY_STEP_3_FOUTPUT_NUM)
    history_output_request = model.HistoryOutputRequest(
        name='H-Output-1',
        createStepName='Step-1',
        variables=PRESELECT,
        numIntervals=MY_STEP_1_FOUTPUT_NUM,
    )
    history_output_request.setValuesInStep(stepName='Step-2', numIntervals=MY_STEP_2_FOUTPUT_NUM)
    history_output_request.setValuesInStep(stepName='Step-3', numIntervals=MY_STEP_3_FOUTPUT_NUM)
...
def M1110_create_contact():
    ...
    structure_bottom_nonbonding_surface = assembly.SurfaceByBoolean(
    ...
    substrate_top_nonbonding_surface = assembly.SurfaceByBoolean(
    ...
    bonding_contact.CohesiveBehavior(eligibility=ALL_NODES)
    ...
    inter.contactPropertyAssignments.appendInStep(
        ...
        assignments=[
            ...
            (substrate_top_bonding_surface, structure_bottom_bonding_surface, 'BONDING'),
            (substrate_top_nonbonding_surface, structure_bottom_nonbonding_surface, 'NONBONDING'),
        ],
    )
...
def M1120_create_bonding_disp_bc():
    ...
    substrate_xneg_disp_bc.setValuesInStep(stepName='Step-3', u1=+MY_SUBSTRATE_PRESTRAIN*MY_SUBSTRATE_X_LEN/2.0)
    substrate_xpos_disp_bc.setValuesInStep(stepName='Step-3', u1=-MY_SUBSTRATE_PRESTRAIN*MY_SUBSTRATE_X_LEN/2.0)
    substrate_yneg_disp_bc.setValuesInStep(stepName='Step-3', u2=+MY_SUBSTRATE_PRESTRAIN*MY_SUBSTRATE_Y_LEN/2.0)
    substrate_ypos_disp_bc.setValuesInStep(stepName='Step-3', u2=-MY_SUBSTRATE_PRESTRAIN*MY_SUBSTRATE_Y_LEN/2.0)
